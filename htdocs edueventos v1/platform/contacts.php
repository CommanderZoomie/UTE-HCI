...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombres) || empty($apellidos) || empty($correo)) {
        ...
        if ($action === 'add') {
            ...
            if ($contactos_id) {
                // Fetch current contact data to retain old photo if new one isn't uploaded
                $current_contact = $pdo->prepare("SELECT contactos_foto FROM contacts WHERE contactos_id = :id");
                $current_contact->execute(['id' => $contactos_id]);
                $old_foto_path = $current_contact->fetchColumn();

                $foto_to_save = empty($foto_path) ? $old_foto_path : $foto_path;

                try {
                    $stmt = $pdo->prepare("UPDATE contacts SET contactos_nombres = :nombres, contactos_apellidos = :apellidos, contactos_institucion = :institucion, contactos_pais = :pais, contactos_ci = :ci, contactos_telefono = :telefono, contactos_correo = :correo, contactos_foto = :foto WHERE contactos_id = :id");
                    $stmt->execute([
                        'nombres' => $nombres,
                        'apellidos' => $apellidos,
                        'institucion' => $institucion,
                    ...
                    <?php foreach ($contacts as $contact): ?>
                        <tr>
                            ...
                            <td class="action-icons">
                                <img src="<?= BASE_URL ?>images/view_icon.png" alt="Ver" title="Ver Detalles" onclick="alert('Ver detalles del contacto: <?= htmlspecialchars($contact['contactos_nombres'] . ' ' . $contact['contactos_apellidos']) ?>');">
                                <img src="<?= BASE_URL ?>images/edit_icon.png" alt="Editar" title="Editar Contacto" onclick="openEditContactModal(<?= htmlspecialchars(json_encode($contact)) ?>);">
                                <img src="<?= BASE_URL ?>images/delete_icon.png" alt="Eliminar" title="Eliminar Contacto" onclick="confirmDelete(<?= $contact['contactos_id'] ?>, '<?= htmlspecialchars($contact['contactos_nombres'] . ' ' . $contact['contactos_apellidos']) ?>');">
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
