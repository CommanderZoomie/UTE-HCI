...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titulo) || empty($direccion) || empty($ciudad) || empty($pais)) {
        ...
        if ($action === 'add') {
            ...
            if ($ubicacion_id) {
                try {
                    $stmt = $pdo->prepare("UPDATE locations SET ubicacion_titulo = :titulo, ubicacion_direccion = :direccion, ubicacion_ciudad = :ciudad, ubicacion_pais = :pais, ubicacion_link = :link WHERE ubicacion_id = :ubicacion_id");
                    $stmt->execute([
                        'titulo' => $titulo,
                        'direccion' => $direccion,
                        'ciudad' => $ciudad,
                    ...
                    <?php foreach ($locations as $location): ?>
                        <tr>
                            ...
                            <td class="action-icons">
                                <img src="<?= BASE_URL ?>images/view_icon.png" alt="Ver" title="Ver Detalles" onclick="alert('Ver detalles de la ubicación: <?= htmlspecialchars($location['ubicacion_titulo']) ?>');">
                                <img src="<?= BASE_URL ?>images/edit_icon.png" alt="Editar" title="Editar Ubicación" onclick='openEditLocationModal(<?= json_encode($location, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT) ?>);'>
                                <img src="<?= BASE_URL ?>images/delete_icon.png" alt="Eliminar" title="Eliminar Ubicación" onclick="confirmDelete(<?= $location['ubicacion_id'] ?>, '<?= htmlspecialchars($location['ubicacion_titulo']) ?>');">
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
